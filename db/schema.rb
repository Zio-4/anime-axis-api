# This file is auto-generated from the current state of the database. Instead
# of editing this file, please use the migrations feature of Active Record to
# incrementally modify your database, and then regenerate this schema definition.
#
# This file is the source Rails uses to define your schema when running `bin/rails
# db:schema:load`. When creating a new database, `bin/rails db:schema:load` tends to
# be faster and is potentially less error prone than running all of your
# migrations from scratch. Old migrations may fail to apply correctly if those
# migrations use external dependencies or application code.
#
# It's strongly recommended that you check this file into your version control system.

ActiveRecord::Schema.define(version: 2021_10_04_182157) do

  # These are extensions that must be enabled in order to support this database
  enable_extension "plpgsql"

  create_table "anime_lists", force: :cascade do |t|
    t.bigint "anime_id"
    t.bigint "user_id", null: false
    t.datetime "created_at", precision: 6, null: false
    t.datetime "updated_at", precision: 6, null: false
    t.index ["anime_id"], name: "index_anime_lists_on_anime_id"
    t.index ["user_id"], name: "index_anime_lists_on_user_id"
  end

  create_table "animes", force: :cascade do |t|
    t.string "title"
    t.string "image_url"
    t.integer "score"
    t.string "status"
    t.string "type"
    t.integer "mal_id"
    t.datetime "created_at", precision: 6, null: false
    t.datetime "updated_at", precision: 6, null: false
  end

  create_table "comments", force: :cascade do |t|
    t.bigint "forum_post_id", null: false
    t.bigint "user_id", null: false
    t.text "content"
    t.datetime "created_at", precision: 6, null: false
    t.datetime "updated_at", precision: 6, null: false
    t.index ["forum_post_id"], name: "index_comments_on_forum_post_id"
    t.index ["user_id"], name: "index_comments_on_user_id"
  end

  create_table "forum_posts", force: :cascade do |t|
    t.bigint "forum_id"
    t.bigint "user_id", null: false
    t.bigint "anime_id"
    t.bigint "manga_id"
    t.string "title"
    t.text "content"
    t.datetime "created_at", precision: 6, null: false
    t.datetime "updated_at", precision: 6, null: false
    t.index ["anime_id"], name: "index_forum_posts_on_anime_id"
    t.index ["forum_id"], name: "index_forum_posts_on_forum_id"
    t.index ["manga_id"], name: "index_forum_posts_on_manga_id"
    t.index ["user_id"], name: "index_forum_posts_on_user_id"
  end

  create_table "forums", force: :cascade do |t|
    t.string "name"
    t.datetime "created_at", precision: 6, null: false
    t.datetime "updated_at", precision: 6, null: false
  end

  create_table "manga_lists", force: :cascade do |t|
    t.bigint "manga_id"
    t.bigint "user_id", null: false
    t.datetime "created_at", precision: 6, null: false
    t.datetime "updated_at", precision: 6, null: false
    t.index ["manga_id"], name: "index_manga_lists_on_manga_id"
    t.index ["user_id"], name: "index_manga_lists_on_user_id"
  end

  create_table "mangas", force: :cascade do |t|
    t.string "title"
    t.string "image_url"
    t.integer "score"
    t.string "status"
    t.string "type"
    t.integer "mal_id"
    t.datetime "created_at", precision: 6, null: false
    t.datetime "updated_at", precision: 6, null: false
  end

  create_table "users", force: :cascade do |t|
    t.string "username"
    t.string "password_digest"
    t.text "bio"
    t.binary "avatar"
    t.string "email"
    t.datetime "created_at", precision: 6, null: false
    t.datetime "updated_at", precision: 6, null: false
  end

  add_foreign_key "anime_lists", "animes"
  add_foreign_key "anime_lists", "users"
  add_foreign_key "comments", "forum_posts"
  add_foreign_key "comments", "users"
  add_foreign_key "forum_posts", "animes"
  add_foreign_key "forum_posts", "forums"
  add_foreign_key "forum_posts", "mangas"
  add_foreign_key "forum_posts", "users"
  add_foreign_key "manga_lists", "mangas"
  add_foreign_key "manga_lists", "users"
end
